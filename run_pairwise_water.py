...
if __name__ == "__main__":
    opts, args = getopt(sys.argv[1:], "t:b:d:f:n:", ["test-data=", "base-data=", "dest-data=", "fraction=", "n-sample="])
    output = {}
    for o, a in opts:
        if o in ["-t", "--test-data"]:
            output["test-data"] = a
            ...
            output["fraction"] = float(a)
        elif o in ["-n", "--n-sample"]:
            output["n-sample"] = int(a)
        else:
    ...
    n_samples = output.get("n-sample", False)
    if n_samples:
        prediction_df = prediction_df.sample(n=n_samples)
    if not n_samples:
        frac = output.get("fraction", 1)
        if frac < 1 and frac > 0:
            prediction_df = prediction_df.sample(frac=frac)
    
    training_df = pd.read_csv(training_data)
    test_water_scores = []
    test_total_water_scores = []
    test_avg_water_scores = []
    # test_water_scores_str = []
    for i, r in tqdm(prediction_df.iterrows(), total=prediction_df.shape[0], desc="Running Water Algorithm"):
        test_seq = merge_kmer(r["sequence"].split(" "))
        water_scores = []
        ...
        test_total_water_scores.append(np.sum(water_scores))
        test_avg_water_scores.append(np.average(water_scores))
    ...
    data = {
        ...
        "total-score": test_total_water_scores,
        "average-score": test_avg_water_scores
    }
