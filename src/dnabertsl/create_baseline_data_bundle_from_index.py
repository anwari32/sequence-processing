...
def generate_data(gene_index, source_gene_dir: str|Path, bundle_dest_dir: str|Path, window_size: int, stride: int, ss_only: bool = False, make_kmer=False):
    ...
    for _, r in tqdm(index_df.iterrows(), total=index_df.shape[0], desc="Processing Gene"):
        gene_file = os.path.join(source_gene_dir, str(r["chr"]), str(r["gene"]))
        ...
        for p, q in gene_df.iterrows():
            ...
            for i in range(0, len_sequence - window_size + stride, stride):
                ...
                if make_kmer:
                    subsequence =  kmer(subsequence, 3, 1)
                    sublabel = kmer(sublabel, 3, 1)
